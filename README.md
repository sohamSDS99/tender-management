...
| **Germany — Öffentlicher Einkauf** | `GET https://oeffentlichevergabe.de/api/notice-exports?pubDay=…&format=ocds.zip` | none (CC0) | one ZIP per day | The federal aggregator over 30+ German platforms (evergabe.de, DTVP, subreport, the Länder marketplaces), and the only source here carrying **below-EU-threshold** German notices — a UVgO award never reaches TED. Served only as a ZIP (every uncompressed representation answers HTTP 406). UVgO notices carry no `tenderPeriod`, so the deadline is parsed out of the prose, and most carry no CPV, which makes the German vocabulary in `relevance_profiles.yaml` load-bearing for this source. |
| **Spend Network — Open Opportunities** | `GET https://api.spendnetwork.cloud/api/v3/notices/records_openopps` | **`SPEND_NETWORK_EMAIL` + `SPEND_NETWORK_PASSWORD`** | `offset`/`limit` (max 100; offset caps at 9 900), walked one day at a time | Cross-border aggregator: one account covers ~40 national portals — German DTVP and oeffentlichevergabe, French BOAMP/achatpublic/marchesonline, Irish and Northern Irish eTenders, Canadian bidsandtenders, AusTender, PhilGEPS, US opengov — most of which have no connector of their own. The credential is an **account, not a key**: the connector signs in at `/api/v3/login/access-token` and caches the bearer token (they last eight days). **The whole feed is taken and filtered here**, on this repo's own `PREFILTER_TERMS`, like the UK feeds and CanadaBuys — 735–6 239 notices a day, mean 3 995 over 24 measured days. The window is walked **one day per query** because paging stops dead at 10 000 records. The account meters *requests*, not rate: 25 tripped it flat-out and 25 tripped it again spread over 93 s, then it refused everything for ~5 min and refilled to 35 — so ~30 requests per 5 min, a day of feed ≈ 7 min, and **every request made while blocked extends the block**, which is why a 403 is waited out in silence and the interrupted page retried at the same offset. One keyword-filtered query (phrases quoted — unquoted the API ORs bare words, 4 770 hits vs 23) runs at the end as a cheap net for notices aggregated late, whose *upstream* publication date falls outside the window. |
