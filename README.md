...
| **Spend Network — Open Opportunities** | `GET https://api.spendnetwork.cloud/api/v3/notices/records_openopps` | **`SPEND_NETWORK_EMAIL` + `SPEND_NETWORK_PASSWORD`** | `offset`/`limit` (max 100; offset caps at 9 900) | Cross-border aggregator: one account covers ~40 national portals — German DTVP and oeffentlichevergabe, French BOAMP/achatpublic/marchesonline, Irish and Northern Irish eTenders, Canadian bidsandtenders, AusTender, PhilGEPS, US opengov — most of which have no connector of their own. The credential is an **account, not a key**: the connector signs in at `/api/v3/login/access-token` and caches the bearer token (they last eight days). `search_term__is` is a real server-side search but **ORs bare words**: unquoted, `safety data sheet` matched 4 770 notices in a fortnight; quoted, 23 — so every phrase is quoted and `OR`-joined, which fits the whole keyword list in one request. Unknown parameters are accepted and silently ignored. The only date filter is the **upstream portal's** publication date, not the date Spend Network ingested the notice, so the window is widened by `SPEND_NETWORK_BACKFILL_DAYS` and the repeats deduplicate in ingest. The account is metered and answers a bare HTML `403` when spent — with no `Retry-After`, and every request made while blocked extends the block — so pages are paced and a refusal costs one wait and one retry, never a poll. |
